...
void wienerFilter(unsigned char* _img, int _width, int _height, int dimensions)
{
    int bufSize = dimensions == 5 ? 8 : 4;
    unsigned* m_buf[bufSize];
    unsigned* sn2_buf[bufSize];
    unsigned char g;
    int x;
    int y;
        ...
        if (y >= 0)
        {
            for (x = 0; x < _width; x++)
            {
                ...
                if (sn2)
                {
                    ...
                    { _img[y * _width + x] = (unsigned char) (((m << 1) + 25) / 50); }
                }
            }
        }
    }
...
void qr_wiener_filter(unsigned char* _img, int _width, int _height)
{
    ...
    for (y = -2; y < _height; y++)
    {
        ...
        for (x = -2; x < _width; x++)
        {
            ...
            unsigned m2;
            m = m2 = 0;
            if (y >= 0 && y < _height - 2 && x >= 0 && x < _width - 2)
            {
                for (i = 0; i < 3; i++)
                    for (j = 0; j < 3; j++)
                    {
        ...
        if (y >= 0)
        {
            for (x = 0; x < _width; x++)
            {
                ...
                if (sn2)
                {
                    ...
                    { _img[y * _width + x] = (unsigned char) (((m << 1) + 9) / 18); }
                }
            }
        }
    }
                          ...
                          unsigned _b, int _nb, int _delta, int _ndelta)
{
    ...
    for (g = 0; g < 256; g++)
    {
        ...
        d = QR_GATOS_Q * delta * (QR_GATOS_P2 + (1 - QR_GATOS_P2) /
                                                (1 + exp(2 * (1 + QR_GATOS_P1) / (1 - QR_GATOS_P1) -
                                                         4 * g / (b * (1 - QR_GATOS_P1)))));
        if (d < 1)
        { d = 1; }
        else if (d > 0xFF)
