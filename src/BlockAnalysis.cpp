...
void BlockAnalysis::countLinesInImg(QImage* _image)
{
    ...
    for (int i = _image->rect().top() + _image->rect().height() / 8;
         i < _image->rect().height() - _image->rect().height() / 8; i++)
    {
        QRgb* line = (QRgb * )(m_image->scanLine(i));
        lines[i] = line;
        for (int j = _image->rect().left() + _image->rect().width() / 8;
             j < _image->rect().width() - _image->rect().width() / 8; j++)
        {
